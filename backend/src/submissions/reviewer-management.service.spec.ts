...
describe('ReviewerManagementService', () => {
  ...
  describe('inviteReviewer', () => {
    it('should send an invitation email', async () => {
      (mockPrisma.event.findUnique as jest.Mock).mockResolvedValue({ id: 'ev-1', name: 'Event 1' });
      (mockPrisma.reviewerInvitation.findUnique as jest.Mock).mockResolvedValue(null);

      const result = await service.inviteReviewer('ev-1', 'test@test.com', 'user-1');
    ...
    it('should throw NotFoundException if event is missing', async () => {
        ...
        await expect(service.inviteReviewer('ev-1', 'test@test.com', 'user-1')).rejects.toThrow(NotFoundException);
    });
  });

  describe('manualRegister', () => {
      it('should create a new reviewer with temporary password', async () => {
          (mockPrisma.event.findUnique as jest.Mock).mockResolvedValue({ id: 'ev-1', tenantId: 't-1' });
      ...
      it('should add existing user to committee', async () => {
          (mockPrisma.event.findUnique as jest.Mock).mockResolvedValue({ id: 'ev-1' });
          (mockPrisma.user.findUnique as jest.Mock).mockResolvedValue({ id: 'existing-u-1' });
  ...
  describe('acceptInvitation', () => {
      ...
      it('should throw BadRequest if invitation already accepted', async () => {
          (mockPrisma.reviewerInvitation.findUnique as jest.Mock).mockResolvedValue({ status: InvitationStatus.ACCEPTED });
  ...
  describe('getInvitation', () => {
      it('should return public invitation info', async () => {
          (mockPrisma.reviewerInvitation.findUnique as jest.Mock).mockResolvedValue({ email: 'test@ex.com' });
      ...
      it('should throw NotFound if invitation missing', async () => {
          (mockPrisma.reviewerInvitation.findUnique as jest.Mock).mockResolvedValue(null);
          await expect(service.getInvitation('token-1')).rejects.toThrow(NotFoundException);
      });
  });
});
