...
describe('SubmissionConfigService', () => {
  ...
  describe('updateConfig', () => {
    it('should update event config', async () => {
      ...
      (mockPrisma.event.update as jest.Mock).mockResolvedValue({ id: 'event-1', submissionsEnabled: true });

      const result = await service.updateConfig('tenant-1', 'event-1', { submissionsEnabled: true, submissionStartDate: '2025-01-01' });
    ...
    it('should throw NotFoundException if event not found during update', async () => {
      (mockPrisma.event.findFirst as jest.Mock).mockResolvedValue(null);
      await expect(service.updateConfig('tenant-1', 'event-1', {})).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('Thematic Areas', () => {
    it('should create thematic area', async () => {
      (mockPrisma.event.findFirst as jest.Mock).mockResolvedValue({ id: 'event-1' });
      (mockPrisma.thematicArea.create as jest.Mock).mockResolvedValue({ id: 'area-1', name: 'AI' });

      const result = await service.createThematicArea('tenant-1', 'event-1', { name: 'AI' });
      expect(result.name).toBe('AI');
    });

    it('should delete thematic area', async () => {
      (mockPrisma.event.findFirst as jest.Mock).mockResolvedValue({ id: 'event-1' });
      (mockPrisma.thematicArea.findFirst as jest.Mock).mockResolvedValue({ id: 'area-1' });
    ...
    it('should throw NotFound if thematic area not found during delete', async () => {
      ...
      await expect(service.deleteThematicArea('tenant-1', 'event-1', 'area-1')).rejects.toThrow(NotFoundException);
    });
  });
  ...
  describe('Rules', () => {
    it('should create rule and upload file', async () => {
      (mockPrisma.event.findFirst as jest.Mock).mockResolvedValue({ id: 'event-1' });
      (mockMinio.uploadObject as jest.Mock).mockResolvedValue('http://minio/rule.pdf');

      await service.createRule('tenant-1', 'event-1', { title: 'Rule' }, { buffer: Buffer.from('test'), mimetype: 'pdf' });
      expect(mockPrisma.submissionRule.create).toHaveBeenCalled();
    });

    it('should delete rule', async () => {
      (mockPrisma.event.findFirst as jest.Mock).mockResolvedValue({ id: 'event-1' });
      (mockPrisma.submissionRule.findFirst as jest.Mock).mockResolvedValue({ id: 'rule-1' });
    ...
    it('should throw NotFound if rule not found during delete', async () => {
      (mockPrisma.event.findFirst as jest.Mock).mockResolvedValue({ id: 'event-1' });
      (mockPrisma.submissionRule.findFirst as jest.Mock).mockResolvedValue(null);
      await expect(service.deleteRule('tenant-1', 'event-1', 'rule-1')).rejects.toThrow(NotFoundException);
    });
  });
});
