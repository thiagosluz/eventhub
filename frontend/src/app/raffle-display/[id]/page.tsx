...
export default function RaffleDisplayPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 flex flex-col items-center justify-center p-8 overflow-hidden relative selection:bg-primary/30">
      ...
      <div className="z-10 text-center w-full max-w-5xl">
        <div className="mb-12 flex justify-center">
          <div className="w-24 h-24 bg-white/5 rounded-3xl flex items-center justify-center border border-white/10 backdrop-blur-md shadow-[0_0_50px_rgba(16,185,129,0.2)]">
            <TrophyIcon className="w-12 h-12 text-yellow-400" />
          </div>
        </div>

        <h1 className="text-3xl md:text-5xl font-black text-transparent bg-clip-text bg-gradient-to-br from-white to-white/70 tracking-tight mb-12">
          SORTEIO OFICIAL
        </h1>

        <div className="min-h-[350px] flex items-center justify-center w-full">
          {isSpinning ? (
            <div className="text-6xl md:text-8xl w-full px-4 overflow-hidden whitespace-nowrap text-ellipsis font-black text-white/50 animate-pulse tracking-tighter mix-blend-screen transition-all">
              {dummyName}
            </div>
          ) : displayWinner ? (
            <div className="flex flex-col items-center animate-in zoom-in slide-in-from-bottom-5 duration-700">
              ...
              <div className="text-6xl md:text-[8rem] w-full px-4 overflow-hidden whitespace-nowrap text-ellipsis font-black text-white tracking-tighter drop-shadow-[0_0_35px_rgba(16,185,129,0.6)] leading-tight">
                {displayWinner.name.split(' ')[0]} {/* Destaca primeiro nome grandioso */}
                <span className="block text-4xl md:text-6xl opacity-80 mt-2 tracking-tight">{displayWinner.name.split(' ').slice(1).join(' ')}</span>
              </div>

              {displayWinner.prize && (
                <div className="mt-12 px-10 py-5 bg-emerald-500/20 border border-emerald-500/40 rounded-[2rem] backdrop-blur-lg animate-in fade-in slide-in-from-bottom-10 delay-300 duration-700">
                  <p className="text-2xl md:text-4xl font-bold text-emerald-300 uppercase tracking-widest">
                    Prêmio: <span className="text-white ml-2">{displayWinner.prize}</span>
                  </p>
                </div>
              )}
            </div>
          ) : (
            <div className="text-2xl md:text-4xl font-bold text-white/20 tracking-[0.3em] uppercase animate-pulse">
              Aguardando sorteio...
            </div>
          )}
        </div>
